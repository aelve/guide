...
-- | Fetch a 'Category' or fail.
selectCategory :: Uid Category -> ExceptT DatabaseError Transaction Category
selectCategory catId = do
  mCategory <- selectCategoryMaybe catId
  case mCategory of
    Nothing       -> throwError $ CategoryNotFound catId
    Just category -> pure category

-- | Check if category is archived
isCategoryArchived :: Uid Category -> ExceptT DatabaseError Transaction Bool
isCategoryArchived catId = do
  let statement :: Statement (Uid Category) (Maybe Bool)
      statement = dimap SingleParam (fmap fromSingleColumn) $
        [queryRowMaybe|
          SELECT archived
          FROM categories
          WHERE uid = $1
        |]
  mArchived <- lift $ HT.statement catId statement
  case mArchived of
    Nothing -> throwError $ CategoryNotFound catId
    Just archived -> pure archived

-- | Fetch a list of available or archived categories' IDs.
--
-- Run command twice with different Bool to fetch all categories' IDs.
selectCategoryIds :: "archived" :! Bool -> ExceptT DatabaseError Transaction [Uid Category]
selectCategoryIds (arg #archived -> archived) = do
  let statement :: Statement Bool [Uid Category]
      statement =
        dimap SingleParam  (map fromSingleColumn) $
        [queryRows|
          SELECT uid
          FROM categories
          WHERE archived = $1
        |]
  lift $ HT.statement archived statement

-- | Fetch a list of available or archived categories.
--
-- Run command twice with different Bool to fetch all categories.
selectCategories :: "archived" :! Bool ->  ExceptT DatabaseError Transaction [Category]
selectCategories (arg #archived -> archived) = do
  catIds <- selectCategoryIds (#archived archived)
  traverse selectCategory catIds
