...
insertCategory
  ...
  insertCategoryWithCategory (#archived False) category

-- | Create category passing 'Category' and archived parameter.
insertCategoryWithCategory
  :: "archived" :! Bool
  -> Category
  -> ExceptT DatabaseError Transaction ()
insertCategoryWithCategory (arg #archived -> archived) category = do
  let statement :: Statement (Uid Category, Category, Bool) ()
      statement =
        [execute|
          ...
          VALUES ($1,$2,$3)
        |]
  lift $ HT.statement (categoryUid category, category, archived) statement
